...
class SeatsView : View {
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        val height = getDefaultSize(suggestedMinimumWidth, heightMeasureSpec)
    ...
    private fun drawSeat(canvas: Canvas?, seat: Seat) {
        if (seat.isBooked) {
            ResourcesCompat.getColor(resources, R.color.grey_200, null).let {
                backgroundPaint.color = it
                armrestPaint.color = it
                bottomSeatPaint.color = it
            }
            ...
            ResourcesCompat.getColor(resources, R.color.grey_200, null).let {
                bottomSeatPaint.color = it
                numberSeatPaint.color = it
            }
        }
        canvas?.apply {
            save()
            translate(seat.x as Float, seat.y as Float)
            ...
            armrestPath.apply {
                ...
                drawPath(armrestPath, armrestPaint)
                translate(150F, 0F)
                addRect(0F, 0F, 50F, 200F, Path.Direction.CCW)
                drawPath(armrestPath, armrestPaint)
            }
            ...
            bottomSeatPath.apply {
                addRect(0F, 0F, 200F, 25F, Path.Direction.CCW)
            }
            drawPath(bottomSeatPath, bottomSeatPaint)
            ...
            drawText(seat.name, 100F - mBounds.centerX(), 100F, numberSeatPaint)
            restore()
        }
    }
}
